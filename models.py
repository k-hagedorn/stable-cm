...
def resample(x, f=[1,1], mode='keep'):
    ...
    c = x.shape[1]
    if mode == 'down':
        return th.nn.functional.conv2d(x, f.tile([c, 1, 1, 1]), groups=c, stride=2, padding=(pad,))
    assert mode == 'up'
    return th.nn.functional.conv_transpose2d(x, (f * 4).tile([c, 1, 1, 1]), groups=c, stride=2, padding=(pad,))

#----------------------------------------------------------------------------
# Magnitude-preserving SiLU (Equation 81).

def mp_silu(x):
    return th.nn.functional.silu(x) / 0.596
...
def mp_cat(a, b, dim=1, t=0.5):
    ...
    wa = C / np.sqrt(Na) * (1 - t)
    wb = C / np.sqrt(Nb) * t
    return th.cat([wa * a , wb * b], dim=dim)

#----------------------------------------------------------------------------
# Magnitude-preserving Fourier features (Equation 75).


class MPFourier(th.nn.Module):
...
class MPConv(th.nn.Module):
    def __init__(self, in_channels, out_channels, kernel):
        super().__init__()
        self.out_channels = out_channels
        self.weight = th.nn.Parameter(th.randn(out_channels, in_channels, *kernel))

    def forward(self, x, gain=1):
        w = self.weight.to(th.float32)
        if self.training:
            with th.no_grad():
        ...
        if w.ndim == 2:
            return x @ w.t()
        assert w.ndim == 4
        return th.nn.functional.conv2d(x, w, padding=(w.shape[-1]//2,))





class WeightFunc(th.nn.Module):
    ...
    def forward(self, t):
        ...
        logvar = self.logvar_linear(self.logvar_fourier(t)).reshape(-1, 1, 1, 1)
        return logvar
