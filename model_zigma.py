...
from utils_zigzag import reverse_permut_np, zigzag_path, hilbert_path
...
class TimestepEmbedder(nn.Module):
    ...
    def forward(self, t):
        ...
        self.mlp = self.mlp.to(device=t_freq.device)
...
class Block(nn.Module):
    ...
    def forward(
        ...
        if not self.has_text:
            self.adaLN_modulation = self.adaLN_modulation.to(device=c.device)
            shift_mba, scale_mba, gate_mba = self.adaLN_modulation(c).chunk(3, dim=1)
            x = x + gate_mba.unsqueeze(1).to(device=x.device) * self.mixer(
...
class ZigMa(nn.Module):
    ...
    def __init__(
        ...
        datatype = torch.float16 if use_fp16 else torch.float32
        self.factory_kwargs = factory_kwargs = {"device": device, "dtype": datatype}
        super().__init__()
        ...
        self.t_embedder = (
            TimestepEmbedder(self.embed_dim, dtype=datatype).to(device).to(datatype)
        )
        ...
        if self.use_pe == 1:  # fixed sin-cos embedding
            ...
            self.pos_embed = nn.Parameter(
                torch.zeros(1, num_patches_4pe, embed_dim, device=device, dtype=datatype),
                requires_grad=False,
            )
            ...
            self.pos_embed_list = [
                nn.Parameter(
                    torch.zeros(
                        1, num_patches_4pe, embed_dim, device=device, dtype=datatype
                    )
                )
            ] * depth
        elif self.use_pe == 0:
        ...
        if self.tpe:
            self.temporal_pos_embedding = nn.Parameter(
                torch.zeros(1, video_frames, embed_dim, device=device, dtype=datatype)
            )
        ...
        if has_text:
            self.y_embedder = nn.Linear(d_context, embed_dim).to(device).to(datatype)
    ...
    def convert_to_fp16(self):
        ...
        """
        self.x_embedder.apply(convert_module_to_f16)
        self.blocks.apply(convert_module_to_f16)
        #self.middle_block.apply(convert_module_to_f16)
        self.final_layer.apply(convert_module_to_f16)
